    ...
    def chat(self, query):
        ...
        for content in llm_responses:
            ...
            if last_punct_pos != -1:
                ...
                if segment_text:
                    if "ja:" in segment_text and "zh:" in segment_text:
                                # 如果同时包含zh和ja，则优先处理zh部分
                                language = "ja"
                                text_zh = segment_text.split("zh:")[-1].split("ja:")[0].strip()
                                text_ja = segment_text.split("ja:")[-1].strip()
                    elif "zh:" in segment_text:
                        language = "zh"
                        # 分割文本，取出中文部分
                        text_zh = segment_text.split("zh:")[-1].strip()
                        if len(segment_text.split("zh:")) > 1:
                            text_ja = segment_text.split("zh:")[0].strip()
                        else:
                            text_ja = ""
                    elif "ja:" in segment_text:
                        language = "ja"
                        # 分割文本，取出日文部分
                        text_ja = segment_text.split("ja:")[-1].strip()
                        if len(segment_text.split("ja:")) > 1:
                            text_zh = segment_text.split("ja:")[0].strip()
                        else:
                            text_zh = ""
                    elif language == "ja":
                        # 如果语言是日文，则取出日文部分
                        ...
                        text_zh = segment_text
                        text_ja = ""
                    # 去处字符串中的```
                    text_ja = text_ja.replace("```", "").strip()
                    text_zh = text_zh.replace("```", "").strip()
                    if text_ja is not None and len(text_ja) > 0:
                        text_index += 1
                        self.recode_first_last_text(segment_text, text_index)
                        future = self.executor.submit(
                            self.speak_and_play, text_ja, text_index
                        )
                        self.tts_queue.put(future)
                    if text_zh is not None and len(text_zh) > 0:
                        self.text_zh.append(text_zh)
        ...
        if remaining_text:
            ...
            if segment_text:
                ...
                if text_ja is not None and len(text_ja) > 0:
                    ...
                    self.tts_queue.put(future)
                    while text_index > len(self.text_zh):
                        self.text_zh.append(self.text_zh[-1])
                if text_zh is not None and len(text_zh) > 0:
                    self.text_zh.append(text_zh)
                ...
                if tool_call_flag:
                    ...
                    if last_punct_pos != -1:
                        ...
                        if segment_text:
                            ...
                            text_ja = text_ja.replace("```", "").strip()
                            text_zh = text_zh.replace("```", "").strip()
                            # print("text_ja:", text_ja)
                            # print("text_zh:", text_zh)
                            if language != "unknow":
                                if text_ja is not None and len(text_ja) > 0:
                                    text_index += 1
                                    self.recode_first_last_text(text_ja, text_index)
                                    future = self.executor.submit(
                                        self.speak_and_play, text_ja, text_index
                                    )
                                    self.tts_queue.put(future)
        ...
        while not self.stop_event.is_set():
            ...
            try:
                try:
                    ...
                    continue
                text_zh = ""
                if len(self.text_zh) <= 0:
                    text_zh = text
                elif self.llm_finish_task and text_index == self.tts_last_text_index:
                    # 取出所有的中文
                    text_zh = "".join(self.text_zh)
                    self.text_zh = []
                else:
                    # 取出第一个
                    text_zh = self.text_zh[0]
                    self.text_zh = self.text_zh[1:]
                print("audio_play_priority_thread text_zh:", text_zh)
                future = asyncio.run_coroutine_threadsafe(
                    sendAudioMessage(self, opus_datas, text_zh, text_index), self.loop
                )
                future.result()
            except Exception as e:
                self.logger.bind(tag=TAG).error(
